...
int GetRandomArrival(JAM waktu){
    srand(waktu.SS);
    return (rand() % 27)+15;
}

int GetRandomNCust(JAM waktu){
    srand(waktu.SS);
    if ((rand() % 2) == 0) {
        return 2;
    } else { /* rand() mod 2 == 1 */
...
void UpdateCondition(Player pemain, TabPesanan OrderTab, Queue CustQ){
